...
| 4 bis | Le `correcteur` n'a jamais tourné | Quatre agents ont été éprouvés sur banc avec des pièges plantés : `tdd-writer` 6/6, `verifier` 8/8, `contradicteur` 5/5 deux fois. Le `correcteur` n'a été lancé ni sur banc ni en run — les deux livraisons du 04/09 n'ont produit aucun défaut à corriger. Sa fiche décrit une passe unique, une liste fermée et un test avant correction : rien de tout cela n'est vérifié. | Un banc avec trois défauts plantés : un testable, un visuel, un qui résiste. Vérifier qu'il s'arrête au bout de trois essais et qu'il dit ce qui part sans filet. |
