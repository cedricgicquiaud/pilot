...
| **Un worktree par agent** | Un *worktree* est un second dossier de travail Git branché sur le même dépôt, sur sa propre branche. | `git worktree add ../<nom> -b feature/<CODE>-<n>-<slug> origin/main` — toujours depuis `main`, même en série, pour que les PR restent indépendantes (pas d'empilement, pas de rebase). Si A casse tout dans son dossier, B ne le voit pas. |
