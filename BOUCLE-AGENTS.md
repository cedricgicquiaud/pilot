...
| 11 | Profil de navigateur dédié aux tests | Un gestionnaire de mots de passe a bloqué le testeur sur un champ (essai du 28/08). Un profil Chrome « Tests » sans extension tierce, avec seulement l'extension Claude, supprime ce hasard. Geste humain, cinq minutes. | Créer le profil, le connecter à l'extension ; la fiche `testeur` prend le premier navigateur connecté. |
| 12 | Captures durables | Les captures du testeur sont des identifiants de session, pas des fichiers : elles ne peuvent pas être jointes à la PR comme preuve. | Instrument qui écrit un fichier (Playwright, Maestro) ; joindre les captures au rapport de recette dans la PR. |
| 13 | Recherche et contradicteur au cadrage (repris de FORGE, phase FIND) | La validation du cadrage est aveugle : personne ne voit ce qui n'est pas écrit. FORGE faisait, avant le PRD et avant chaque SPEC, une recherche (état de l'art, standards du marché) puis une critique par un second avis (`/advisor` : cas manquants, contradictions, règles non testables). Les deux ont été perdus dans l'absorption. Coût estimé : un quart d'heure par feature, avant le run. Décision du 28/08 : plus tard, une fois la boucle stabilisée. | Fiche `contradicteur` (mandat « qu'est-ce qui manque ? », grille fixe, plafond 5 par catégorie, réponse fermée « à ajouter / hors périmètre »), appelée par `init` (PRD) et `feature` (contrat). Recherche projet dans `init`, recherche feature ciblée dans `feature`. Mesure : trous du cadrage comptés par `sync` après coup. Source : dépôt `AlanZien/FORGE`, `templates/.claude/rules/phases/01-find.md` et `03-spec.md`. |
