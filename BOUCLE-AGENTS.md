...
| Vue de contrôle (avancement, budget) | V3 | Linear pour l'avancement ; la dépense après coup par le relevé de coût, et en direct depuis le 09/09 (`--direct`) | Couvert le 09/09 |
...
| 2 | Modèle déclaré par agent | **En partie fait le 01/09** : `model:` accepte `opus`, `sonnet`, `haiku`, `fable`, `inherit` ou un identifiant complet. Posé où le choix était évident — `fable` sur `decoupeur` et `contradicteur`, les deux agents de jugement du cadrage, qui tournent une fois par feature sur du texte court (33 et 21 échanges mesurés). **Tranché le 09/09 par le n° 4** : `verifier` reste sur le modèle de la session (relecteur, il contredit) ; `testeur` passe sur `sonnet` en exécutant pur, le jugement sur l'image revenant à l'humain par les écrans dans la PR. `tdd-writer` et `correcteur`, qui écrivent le code, restent à mettre au banc : l'hypothèse « un modèle moins capable contourne davantage » n'a pas été mesurée. | Champ `model` dans les fiches. |
...
| 9 | Vue de contrôle | **En partie traité le 01/09** par `cout-agents.py` : après coup, le coût par agent et les agents à regarder. **Depuis le 07/09** il sépare horloge, temps actif et attente, nomme la commande avant chaque attente de plus de cinq minutes, et retrouve les transcripts par leur dossier de travail même si la session a été lancée d'ailleurs. **Le direct existe depuis le 09/09** : `cout-agents.py . --direct` affiche, dans un second terminal, une ligne par agent en cours — état (actif, bloqué sur une commande, rapport rendu), minutes actives, jetons relus contre son seuil, dernier geste — rafraîchie toutes les trente secondes ; `--notifier` envoie les alertes en notification macOS par cmux ; `--une-fois` donne l'instantané que le lead joint à chaque ligne du fil. Il lit les mêmes journaux que le relevé, pendant qu'ils s'écrivent : aucun agent ne sait qu'on le regarde, aucune fiche ne bouge. Reste à voir en usage réel si l'état « bloqué » se déclenche à tort sur une commande longue (une suite Playwright de neuf minutes le déclenche : c'est voulu, on regarde, on ne coupe pas). | Fait. Le n° 10 s'appuiera sur cette lecture pour décider seul de continuer ou de s'arrêter. |
...
| Le relevé de coût, et la vue en direct pendant un run (`--direct`) | `tools/cout-agents/cout-agents.py` |
