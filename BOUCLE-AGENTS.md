...
| 9 | Vue de contrôle | **En partie traité le 01/09** par `cout-agents.py` : après coup, le coût par agent et les agents à regarder. **Depuis le 07/09** il sépare horloge, temps actif et attente, nomme la commande avant chaque attente de plus de cinq minutes, et retrouve les transcripts par leur dossier de travail même si la session a été lancée d'ailleurs. **Le direct, fait le 09/09 et retiré le 10/09** : `cout-agents.py . --direct` affichait une ligne par agent en cours dans un second terminal. Cédric a montré que Claude Code affiche déjà la même chose, mieux placé : la ligne de chaque agent en bas du fil donne durée, jetons et geste du moment. Refaire cette vue à côté n'apportait rien. Ce qui reste, parce que Claude Code ne le calcule pas : `--direct --notifier` envoie une notification macOS quand un agent est bloqué plus de cinq minutes sur une commande ou dépasse son seuil ; c'est un outil de secours, pas une vue à regarder. Leçon : regarder ce que l'écran montre déjà avant de construire un affichage. | Fait. Le n° 10 s'appuiera sur cette lecture pour décider seul de continuer ou de s'arrêter. |
...
| Le relevé de coût, et les alertes pendant un run (`--direct --notifier`) | `tools/cout-agents/cout-agents.py` |
